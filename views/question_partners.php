               ...
               <div class="col-md-9 mb130 mt-lg-5" role="main">                  
                  <div class="mb30">
                    <div class="container wow fadeInUp" data-wow-delay=".2s">                    
                       <form name="oForm" id="oForm"  method="post" action="<?=base_url('question/partners/'.$line_push)?>"> 
                        <div class="row">                                	
  												<div class="form-group mx-sm-3 mb-2">  											 						   
                               <select name="p_id" id="p_id" class="form-control">
                               	  <?php if (isset($s_list) && count($s_list) > 0){                                	  	        
                               	  	        foreach ($s_list as $key => $item){
                               	  	        	       $selected = '';
                               	  	        	       if ($p_id == $item['p_id']){
                               	  	        	           $selected = ' selected';	
                               	  	        	       }
                               	  	        	       
                               	  	        	       echo '<option value="'.$item['p_id'].'" '.$selected.'>'.trim($item['p_name']).'</option>';
                               	  	        }
                               	        }else{
                               	        	  echo '<option value="">尚無項目</option>';
                               	        }
                               	  ?>
                               </select> 
  											  </div>
...
<form name="PageForm" method="post" action="<?=base_url('question/partners/'.$p_id)?>" style="margin-top: -60px;">
  ...
  ?>  
               ...
               <?php if (isset($member) && count($member)){                	
               	         $item = $member[0];
               	         $n++;  
               	         ?>
                  <div class="card">
               ...
               <aside role="complementary" class="aside col-xl-3 col-md-3 mb130">					
         				  <?=$this->block_service->member_question_menu($line_push); ?>  
               </aside>
			        </div>
			      </div>
			    </div>  
