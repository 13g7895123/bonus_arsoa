    ...
    echo  "<tr><td colspan=8>".$searchstr."</td></tr>";
		...
		foreach ( $list['rows'] as $rs ){
                ...
                <tr class="<?php echo $bgclass?>">
                 <td>
                   <input type="submit" value="回覆" class="btn btn-xs btn-default" onclick="javascript:action='<?php echo base_url( 'wadmin/func/feedback_modify/'.$kind.'/'.$ftype ); ?>?edit=<?php echo $rs["id"]?>';" alt="回覆" border="0" >                   
                      &nbsp;&nbsp;|&nbsp;&nbsp;
                      <input type="checkbox" name="del[]" id="del[]" value="<?php echo $rs["id"]?>">
                 </td>
                 
                 <td><?php echo htmlspecialchars($rs["name"])?></td>
                 <td><?php echo htmlspecialchars($rs["phone"])?></td>
                 <td><a href="mailto:<?php echo $rs["email"]?>"><?php echo $rs["email"]?></a></td>
                 <td><?php echo htmlspecialchars($rs["title"])?></td>
                 <td><?php echo $this->block_service->PF_FDT($rs["crdt"])?></td>
                 <td><?php echo htmlspecialchars($rs["account"])?></td>
                 <td><?php echo $this->block_service->PF_FDT($rs["redate"])?></td>
                </tr>
                <?php }  ?>
</table>
</div>
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel"><?=$this->block_service->PF_SearchXML($this->XmlDoc,"//參數設定檔/權限/選單/SND/傳回值","資料", $kind)?> 資料匯出</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <form name="OutForm" method="post" language="javascript" action="<?php echo base_url( 'wadmin/func/out_download/'.$kind.'/'.$ftype ); ?>" target=_blank>
