...
27. 看到這一頁，目前是進入後就載入資料，幫我調整一下這支載入資料的API，/api/eeform1/submissions/000000，改成在提示視窗選完後才顯示，依據要編輯的表單來顯示，如果他選擇會員，則只顯示identity為member的資料，來賓則顯示guest，等等api內ˇ容請不要修改，一樣用原本的API，只是顯示的時候依據不同條件篩選即可
