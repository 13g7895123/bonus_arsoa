...
class Eeform4 extends MY_Controller
{
     ...
     * GET /api/eeform4/export_grouped?member_name=xxx&member_id=xxx
     */
    public function export_grouped() {
        try {
            ...
            $submissions = $this->eform4_model->get_submissions_by_member_info($member_name, $member_id);
            ...
            foreach ($submissions as $index => $item) {
                ...
                // 表單標題
                $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(0, $row, '會員服務追蹤管理表(保健) - 第'.($index+1).'筆');
                $objPHPExcel->getActiveSheet()->getStyle('A'.$row.':B'.$row)->getFill()
                    ->setFillType(PHPExcel_Style_Fill::FILL_SOLID)
                    ->getStartColor()->setARGB('004472C4');
                $objPHPExcel->getActiveSheet()->getStyle('A'.$row.':B'.$row)->getFont()->setBold(true)->setColor(new PHPExcel_Style_Color('FFFFFFFF'));
