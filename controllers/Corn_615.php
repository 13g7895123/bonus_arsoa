    	           ...
    	           if ($send_data){
    	               foreach ($send_data as $key => $item)
    	               {
                          //    echo "<pre>".print_r($item,true)."</pre>";             
                              ...
                              if ($send_num > 0){  // 第二次的問卷,要看第一次是否已填寫                              	 
                              	  $reply_data = $this->question_model->reply_find_one('rid',$item['last_rid'],'N');                                 	  
                              	  if ($reply_data['status'] == 'P'){ // 已寄過未填
                              	      $have_send = 'Y';
                              	  }
                              ...
                              if ($send_num == 0 && $diff_day > 15){     //送出ap_member_order_his 的資料，若當下該會員沒綁定，保留15天，有綁再傳！過15天就不管了。
                                  ...
                                  foreach ($set_data as $qkey => $qset){
                                  	       ...
                                  	       if (($have_send == 'N' && $qkey == $send_num && $diff_day >= (int)$qset['day']) || ($qkey == $send_num && $diff_day == (int)$qset['day'])){      // 寄送    
                                  	       	    echo 'OK';                                  	       	                  	            
                                  	            if (isset($qset['q_id']) && $qset['q_id'] > 0){
                                  	            	  if (!isset($question_data_set[$qset['q_id']])){
                                  	            	  	  $question_data = $this->question_model->find_one('q_id',$qset['q_id'],true); 
                                  	            	      $question_data_set[$qset['q_id']] = $question_data;
                                  	            	  }else{
                                  	            	      $question_data = $question_data_set[$qset['q_id']];
                                  	            	  }                              	            	  
                                  	            }
