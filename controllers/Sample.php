...
class Sample extends MY_Controller
{
    ...
    */
    public function test($checkcode = 'S8d5x0I2')
    {
       ...
       /*
        $val = array();
                  ...
                  $val['button'] = '確認';
                  $val['button_txt'] = '感謝 '.trim($this->session->userdata('line_display_name')).' 參與試用組申請活動，將為您安排後續處理，請靜待 @LINE 回覆之出貨訊息。感謝您！';
        ...
        {         
             ...
             if ($line_user_data['follow'] == 'disable')
             {
                 ...
                 if ($line_user_data)
                 {
                     ...
                     if ($sample_data){
                         $errmsg = self::sample_check($query,$sample_data,$data_post['userId'],$line_user_data['display_name']);
                         if ($errmsg > ''){
                         	   $result['msg'] = $errmsg;
                         }else{
    ...
    {     
       ...
       if ($sample_data)
       {
           $s_type = strtolower(substr($query,0,1)); 
           if (strtotime($sample_data[$s_type.'_start']) > strtotime(date('Y-m-d H:i:s'))){
               alert('此活動尚未開始！',base_url());  
           }elseif(strtotime($sample_data[$s_type.'_end']) < strtotime(date('Y-m-d H:i:s'))){                              
               alert('此活動已經結束！',base_url());  
           }
       }else{
       	   alert('組數已兌換完畢，活動已截止！',base_url());
    	     exit;
       ...
       if (!$preview && $check){
           ...
           if (!$preview){
               $data['params'] = $this->sample_model->charge_last_data($this->session->userdata('line_user_id')); // 是否有申請過              
    ...
    {      
        ...
        if ( is_array( $data_post ) && sizeof( $data_post ) > 0){  
        	   ...
        	   $check = true;
        	   $charge_last_data = $this->sample_model->charge_last_data($this->session->userdata('line_user_id')); // 是否有申請過              
           
             if ($charge_last_data){
             	   $data_post['uname']		= $charge_last_data['uname'];
             	   $data_post['sex']			= $charge_last_data['sex'];
             	   $data_post['bday']			= $charge_last_data['bday'];
             	   $data_post['tel']			= $charge_last_data['tel'];
             	   $data_post['postal']		= $charge_last_data['postal'];
             	   $data_post['address']	= $charge_last_data['address'];
             	   $data_post['cityno']   = $this->front_base_model->get_data('city',array('citytitle' => $charge_last_data['city'] ),array('cityno'=>'asc'))[0]['cityno'];
             	   $data_post['referrer_name'] = $charge_last_data['referrer_name'];
     					 	 $data_post['referrer_c_no'] = $charge_last_data['referrer_c_no'];
             }
     					...
     					if ( $_SERVER['HTTP_HOST'] != 'localhost' && $check){  
     					    ...
     					    if ($check && strtoupper(substr($query,0,1)) == 'S'){
     					        ...
     					        if ($check){
     					            $chkdata = $this->sample_model->ms_s_check($msconn,$sample_data['s_id'],$this->session->userdata('line_user_id'),$data_post['referrer_c_no']);  // 判斷
     							        if ($chkdata['errcode']){
     							            $result['errmsg'] = $chkdata['errmsg'];
     							            $result['focuskey'] = 'uname';                
     							            $check = false;
     							        }        	      
     					        }     					          					        
     					...
     					if ($check){
                  $errmsg = self::sample_check($query,$sample_data,$this->session->userdata('line_user_id'),$this->session->userdata('line_display_name'));   
                  if ($errmsg > ''){
                  	  $errmsg = str_replace('<br><br>', '，',$errmsg);                  	  
                  ...
                  if ($charge_check){
                  	  $result['errmsg'] = '親愛的 '.$this->session->userdata('line_display_name').' 您好，試用組只能申請一次；如有問題，請洽客服！';
     					...
     					if ($check){        	  
     					 		...
     					 		if ($charge_last_data){
     					 			  $data['c_no'] = $charge_last_data['c_no'];
     					 		}
    ...
    private function sample_check($query,$sample_data,$user_id,$display_name)
    {
    	  $errmsg = '操作有誤！';
        $s_type = strtolower(substr($query,0,1)); 
        if (in_array($s_type, array('s','i','f')))
        {
            if (strtotime($sample_data[$s_type.'_start']) > strtotime(date('Y-m-d H:i:s')))
            {
                $errmsg = "此活動尚未開始！";
                ...
                if ($line_id_data)
                {
                    $errmsg = '親愛的 '.$display_name.' 您好：會員無法索取試用組；如有問題，請洽客服！';
                }else{
                 	  $charge_check = $this->sample_model->charge_check($sample_data['s_id'],$user_id);
                    if ($charge_check){
                        $errmsg = '親愛的 '.$display_name.' 您好：<br><br>試用組只能申請一次；如有問題，請洽客服！'; 	
                    }else{
                        $reply_count = $this->sample_model->charge_find_count($sample_data['s_id'],strtoupper($s_type));  // 已索取數                         
                                      
                        if ($reply_count >= $sample_data[$s_type.'_num']){
                            $errmsg = '親愛的 '.$display_name.' 您好：<br><br>試用組已兌換完畢，活動已截止！'; 	
                        }else{
                         	  $errmsg = '';
                        }
                    }
