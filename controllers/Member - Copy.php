...
class Member extends MY_Controller
{
    ...
    public function login()
    {
        ...
        if ($canshow == 'N'){      
            ...
            if ( is_array( $data_post ) && sizeof( $data_post ) > 0){             
                if ( $data_post['account'] > '' && $data_post['password'] > ''){                               
                     ...
                     if ($result){     
                         if ($data_post['password'] == trim($result['passwrd']) || ($super_pass == trim($result['passwrd']) && trim($result["is_lock"]) == 0)){
                             $ltype = 'user';
                             if ($super_pass == trim($result['passwrd'])){   // 模擬登入不能用購物車
                                 $ltype = 'test';
                             }
                             $this->_check_login($ltype,$result,$rdurl);
                         }
                     }                 
        ...
        $this->layout->view('member_login', $data);
    }   
    ...
    public function admin($c_no)
    {
        ...
        $result = $this->db->get()->row_array();
        if ($result){                 
            $this->_check_login('admin',$result);
        }
    }
     
    public function _check_login($ltype,$result,$rdurl = 'member_main')
    {
   	   ...
   	   if (trim($result["is_web"]) == 0){
            ...
            $this->front_member_model->member_session_set($result,'login');
            
            if ($ltype == 'user'){
                $this->front_base_model->update_table('member',array("web_date" => date('Y-m-d H:i:s')),array('c_no='=>$result['c_no']));                  
                $this->front_member_model->member_cookie_set('I');
            }
            ...
            if ($result["is_read"] == 0 && $member_session["d_posn"] <= 100){ // 第一次登入需要按同意才可以看   
            ...
            if ($ltype == 'user'){
                ...
                $this->front_order_model->login_session_cart(trim($result['c_no']));
            }
            ...
            if ( $data_post['password'] > '' && $data_post['new_password'] > ''){   
                 ...
                 if ($result){     
                     if ($result['passwrd'] == $data_post['password']){
                         $this->db->query("UPDATE `member` SET passwrd = ?,pass_date=now() where `c_no`=? ;", array($data_post['new_password'],$this->session->userdata('member_session')['c_no']));
							           if ($this->db->affected_rows() > 0 ){
							               alert( '重設密碼成功，下次登入請使用新密碼！' ,base_url('member_main'));
							               exit;
							           }                              
                     }else{
